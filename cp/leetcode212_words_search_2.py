...
class Solution(object):
    def findWords(self, board, words):
        ...
        for word in words:
            ...
            while i < len(board):
                ...
                while j < len(board[i]):
                    if board[i][j] == word[0]:
                        temp = list(copy.deepcopy(board))
                        if self.find(temp, i, j, word, 0):
                            output.append(word)
                        del temp
                    j += 1
                i += 1
    ...
    def find(self, board, r, c, word, i):
        if  i >= len(word):
            return True
        else:
            if 0 <= r < len(board) and 0 <= c < len(board[0]):
                if board[r][c] == word[i]:
                    ...
                    board[r][c] = ""
                    if self.find(board, r, c+1, word, i + 1):
                        return True
                    elif self.find(board, r+1, c, word, i + 1):
                        return True
                    elif self.find(board, r, c-1, word, i + 1):
                        return True
                    elif self.find(board, r-1, c, word, i + 1):
                        return True
                    else:
